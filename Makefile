...
release/major: ## Create a major release
	@VERSION="major" $(MAKE) release/create

release/minor: ## Create a minor release
	@VERSION="minor" $(MAKE) release/create

release/patch: ## Create a patch release
	@VERSION="patch" $(MAKE) release/create

release/create: export GITHUB_TOKEN := $(shell gh auth token)
release/create:
	@if [[ -z "$(VERSION)" ]]; then $(call MK_ERROR,"Missing release version"); exit 1; fi
